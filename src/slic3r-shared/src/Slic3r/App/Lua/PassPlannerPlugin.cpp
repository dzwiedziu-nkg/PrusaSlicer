...
using PassPlanner::SurfaceInfo;

/**
 * @brief Stable identifiers for the extrusion roles, as seen by a plugin.
 *
 * Deliberately the enum names rather than gcode_extrusion_role_to_string(), whose output
 * is translated and meant for the UI.
 */
...
std::optional<Plugin> find_plugin(const std::vector<std::string>& plugin_paths)
{
    ...
    for (const Plugin& plugin : registry.plugins() | std::views::values) {
        if (plugin.meta().type != PluginType::SlicingPassPlanner) {
            continue;
        }
        if (found.has_value()) {
            SPDLOG_WARN(
                "Pass planner plugin {} is ignored, {} is already in use",
                plugin.meta().id,
                found->meta().id
            );
            continue;
...
/**
 * @brief Runs one plugin's plan_pass() as a PassPlanner::Strategy.
 *
 ...
 *
 * The support toolpaths are generated from a tbb::parallel_for over layers, so operator()
 * is entered concurrently and every touch of the Lua state is under m_mutex.
 */
class Runner
{
public:
    explicit Runner(Plugin plugin) : m_plugin(std::move(plugin))
    {
        ...
        m_plugin.load(m_lua);
        m_plan_pass = m_lua.state()["plan_pass"];
    }
    ...
    ~Runner()
    {
        if (m_planned > 0) {
            SPDLOG_INFO(
                "Pass planner plugin {} ran an extra pass over {} of {} surfaces",
                m_plugin.meta().id,
                m_planned,
                m_seen
            );
    ...
    std::optional<PassPlanner::Plan> operator()(const SurfaceInfo& surface)
    {
        ...
        sol::table argument = lua.create_table(0, 12);
        ...
        argument["extrusion_width"] = surface.extrusion_width;
        ...
        const sol::protected_function_result result{m_plan_pass(argument)};
        if (!result.valid()) {
            const sol::error error = result;
            disable(fmt::format("plan_pass() failed: {}", error.what()));
            return std::nullopt;
        }

        const sol::object returned = result;
        if (returned.get_type() == sol::type::nil) {
            // The plugin did not want an extra pass over this area.
            return std::nullopt;
        }
        if (returned.get_type() != sol::type::table) {
            disable("plan_pass() answered with neither a table nor nil");
            return std::nullopt;
        }

        std::optional<PassPlanner::Plan> plan = to_plan(returned.as<sol::table>());
        if (!plan.has_value()) {
            disable("plan_pass() answered with something that is not a list of paths");
            return std::nullopt;
        }
        ++m_planned;
        return plan;
...
private:
    ...
    static std::optional<PassPlanner::Plan> to_plan(const sol::table& answer)
    {
        ...
        if (paths_field.get_type() == sol::type::none
            ...
            return PassPlanner::Plan{std::move(*paths)};
        }
        ...
        PassPlanner::Plan plan{std::move(*paths)};
        const sol::object spacing = answer["spacing"];
        if (spacing.get_type() != sol::type::none && spacing.get_type() != sol::type::nil) {
            if (spacing.get_type() != sol::type::number) {
                return std::nullopt;
            }
            plan.spacing = spacing.as<double>();
        }
    ...
    void disable(const std::string& reason)
    {
        m_disabled = true;
        SPDLOG_ERROR(
            "Pass planner plugin {} disabled for this slice: {}", m_plugin.meta().id, reason
        );
    }
...
private:
    ...
    Plugin m_plugin;
    sol::protected_function m_plan_pass;
    std::mutex m_mutex;
...
PassPlanner::Strategy make_pass_planner(const std::vector<std::string>& plugin_paths)
{
    ...
    try {
        ...
        SPDLOG_ERROR(
            "Loading pass planner plugin failed in script {}\n{}", e.script_path(), e.what()
        );
        return {};
    } catch (const std::exception& e) {
        SPDLOG_ERROR("Loading pass planner plugin failed\n{}", e.what());
        return {};
    }

    SPDLOG_INFO("Pass planner plugin in use: {}", runner->id());
