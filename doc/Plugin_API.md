...
The recognized `required_apis` keys are `project.plugin`, `slicing.island_order`, and
`slicing.island_sequence`, all at version `1.0.0`.
...
- `type` (string) type of plugin, allowed values are `'project.plugin'`,
  `'slicing.island_order'`, and `'slicing.island_sequence'`.
